...
const FavoriteRecipesPanel: React.FC<FavoriteRecipesPanelProps> = ({
  ...
  useEffect(() => {
    if (searchTerm.trim() === "") {
      ...
      const filtered = likedRecipes.filter(recipe => recipe.title.toLowerCase().includes(searchTerm.toLowerCase()) || recipe.description.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  if (likedRecipes.length === 0) {
    return <div className="rounded-lg bg-white/80 backdrop-blur-sm shadow-sm p-6 text-center">
  ...
  return <motion.div initial={{
    ...
    y: 0
      ...
      <div className="relative">
        ...
        <Input type="text" placeholder="Search favorite recipes..." value={searchTerm} onChange={handleSearchChange} className="pl-10 bg-white/80 backdrop-blur-sm" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        <AnimatePresence>
          ...
          duration: 0.2
              ...
              <Card className={cn("overflow-hidden transition-all duration-200 hover:shadow-md", "border-l-4 border-l-[#4F2D9E]")}>
                        ...
                        <DialogContent className="sm:max-w-[500px]">
                          <div className="space-y-4 p-4">
                            <div className="flex items-start gap-4">
                              ...
                              <div>
                                <h2 className="text-xl font-semibold text-[#4F2D9E]">
                                  {recipe.title}
                                </h2>
                                <p className="text-gray-600">{recipe.description}</p>
                                {recipe.prepTime && <p className="text-sm text-gray-500 flex items-center mt-1">
                            ...
                            {selectedDate && onAddToPlanner && <div className="flex justify-end mt-4">
                                <Button onClick={() => {
